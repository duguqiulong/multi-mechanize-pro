...
def run_test(remote_starter=None):
    ...
    for i, ug_config in enumerate(user_group_configs):
        ug = core.UserGroup(queue, i, ug_config.name, ug_config.num_threads, ug_config.script_file, ug_config.start_threads, ug_config.increase_threads_every, run_time, rampup)
...
def configure(project_name, config_file=None):
    ...
    for section in config.sections():
        if section == 'global':
            ...
            start_threads = config.get(section, 'start_threads')
            increase_threads_every = config.get(section, 'increase_threads_every')
            user_group_name = section
            ug_config = UserGroupConfig(threads, user_group_name, script,start_threads,increase_threads_every)
...
class UserGroupConfig(object):
    def __init__(self, num_threads, name, script_file,start_threads,increase_threads_every):
        self.num_threads = num_threads
        self.name = name
        self.script_file = script_file
        self.start_threads = start_threads
        self.increase_threads_every = increase_threads_every

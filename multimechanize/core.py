...
class UserGroup(multiprocessing.Process):
    def __init__(self, queue, process_num, user_group_name, num_threads, script_file,start_threads,increase_threads_every, run_time, rampup):
        ...
        self.increase_threads_every = int(increase_threads_every)
    ...
    def run(self):
        threads = []
        delay_list = self.get_delayList()
        for i in range(self.num_threads):
            spacing = float(self.rampup) / float(self.num_threads)
            if i > 0:
                time.sleep(spacing)
            agent_thread = Agent(self.queue, self.process_num, i, self.start_time, self.run_time, self.user_group_name, self.script_file, delay_list[i])
    ...
    def get_delayList(self):
        delay_list = []
        delay_level = 1
        for i in range(self.num_threads):
            delay_list.append(self.increase_threads_every * (delay_level - 1))
            if i >= self.start_threads * delay_level - 1:
                delay_level += 1
            print(i)
        return delay_list
...
class Agent(threading.Thread):
    def __init__(self, queue, process_num, thread_num, start_time, run_time, user_group_name, script_file,start_delay):
